...
`更新する.bat` が `'-NoProfile' is not recognized as an internal or external command` というエラーで止まる問題を修正しました。Windowsのコマンドプロンプトが日本語を含む行の読み位置を見失い、行の途中から実行していたことが原因です。入口のファイルから日本語を完全に取り除いて解決しました。

7月29日版への更新に失敗した方も、今回の更新フォルダでもう一度お試しください。

### 視聴画面を改善

- チャットと概要説明にある `https://`、`http://`、あぷ小の `fu7043289.jpg` 形式、あぷの `f351766.png` 形式を自動でリンクにします。`javascript:` など、それ以外の形式はリンクにしません。
- 別窓・全画面・シアターでは、画面上部にマウスを乗せたときだけ操作ボタンを表示します。タッチ操作の画面では従来どおり常時表示します。
- シアターでは操作ボタンが場所を取らなくなり、映像を画面いっぱいに表示します。

### 再生の自動復帰を改善

立て直しに何度か失敗しても、60秒待って再試行するようになりました（最大3回）。裏で重い処理が終われば、ページを開き直さなくても再生が戻ります。

### 不具合報告と診断を改善

- 約700字あった報告文を約100〜150字に短縮し、コメント欄の初期設定のまま貼れるようにしました。
- 症状が変動する場合は、`直近60秒の揺れ 手元3.0-6.1` のような振れ幅を自動で付けます。
- 管理画面「ログ・診断」の先頭に、視聴者から届いた報告の数字を判断するための表を追加しました。視聴品質メトリクスとログの絞り込みタグにも同じ説明があります。

### 配信者向け対処ガイドを追加

`困ったときの対処ガイド.html` を同梱しました。映らない、止まる、遅れる、コメントが流れない、外から見えないなど12の症状について、「切り分け → 対処 → 視聴者への案内文」を確認できます。視聴者へそのまま送れる案内文も6種類収録しています。

### そのほかの修正

- MediaMTXのポートを既定の8888から変更した場合に、画面表示と実際の接続先が食い違って再生できない問題を修正しました。
- 管理画面と説明書にあった「報告の番号を過去の配信から探せます」という誤った記載を修正しました。報告の記録は保存していないため、過去の番号は検索できません。
